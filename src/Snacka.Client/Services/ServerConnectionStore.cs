...
public class ServerConnectionStore : IServerConnectionStore
{
    ...
    public ServerConnectionStore()
    {
        ...
        var snackaDir = string.IsNullOrEmpty(profile)
            ? Path.Combine(appData, "Snacka")
            : Path.Combine(appData, "Snacka", $"profile-{profile}");
        Directory.CreateDirectory(snackaDir);
        _filePath = Path.Combine(snackaDir, "servers.json");
        Load();
    }
